...
source "virtualbox-iso" "buster64" {
  ...
  output_filename = "packer_{{.BuildName}}_{{.Provider}}"
}

source "virtualbox-ovf" "buster64-lightdm-mate" {
  source_path = "target/packer_{{.BuildName}}_{{.Provider}}.ovf"
  ssh_username = "vagrant"
  ssh_password = "vagrant"
...
build {
  sources = [
    "sources.virtualbox-ovf.buster64-lightdm-mate"
  ]
  ...
  post-processor "vagrant" {
    ...
    provider_override = "virtualbox"
    compression_level = 9
    output = "target/packer_{{.BuildName}}_{{.Provider}}-lightdm-mate.box"
  }
}
